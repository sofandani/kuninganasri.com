...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	$defaults = array('add_below' => 'comment', 'respond_id' => 'respond', 'reply_text' => __('Reply'),
		'login_text' => __('Login to Reply'), 'depth' => 0, 'before' => '', 'after' => '');
	...
	if ( get_option('comment_registration') && !$user_ID )
		$link = '<a rel="nofollow" class="comment-reply-login achor_login_front" href="' . esc_url( wp_login_url( get_permalink() ) ) . '">' . $login_text . '</a>';
	else
...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Reply' );
	if ( false === $replytext ) $replytext = __( 'Reply to %s' );
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="#" class="achor_login_front">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( '<a href="%2$s" title="Edit Profile, %3$s?" class="avatar_comment_form">%1$s</a>' ), get_avatar( $user->ID, '55' ), get_edit_user_link(), $user_identity ) . '</p>',
	...
	?>
		<?php if ( comments_open( $post_id ) ) : ?>
			...
			<div id="respond" class="comment-respond">
				...
				<?php else : ?>
				<h3 id="reply-title" class="comment-reply-title" align="right"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?>?</h3>
					
					<?php do_action( 'attach_stuff_before_comment_form' ); ?>
					<form action="<?php echo site_url( '/comments.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
