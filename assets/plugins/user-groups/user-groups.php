...
class KWS_User_Groups {
	...
	function KWS_User_Groups() {
		...
		add_action('admin_print_styles', array(&$this, 'css_ug_admin'));

		add_action('wp_print_styles', array(&$this, 'cs_ug_front'));
		...
		add_shortcode('group-list', array(&$this, 'group_list_shortcode'));

	}

	function css_ug_admin(){
		wp_register_style( 'user-groups-css', plugins_url('user-groups.css', __FILE__),10000 );
		wp_enqueue_style('user-groups-css', array('jquery'));
	}

	function cs_ug_front(){
		wp_enqueue_style('user-groups-front', plugins_url('user-groups-front.css',__FILE__), 10);
	}

	function group_list_shortcode( $atts ) {
	    global $wpdb;
	    ...
	    if ($userIDs) {
	        $content = '<div class="clear"></div><div class="group-list front-end-type"><ul>';
	        foreach( $userIDs as $userID ) {
	            $user = get_user_by('id', $userID);
	            $content .= '<li id="users-'.$user->user_nicename.'" class="user-groups-lists">';
	            $content .= str_replace("class='avatar", "class='avatar alignleft", get_avatar( $user->ID, 70 ));
	            $content .= '<h2><a href="'.esc_url( get_author_posts_url( $user->ID ) ).'" rel="me">'.$user->display_name.'</a></h2>';
	            $content .= '<p class="description-users-groups">'.get_user_meta($user->ID, 'description', true).'</p>';
	            $content .= '<div class="author-meta"><div class="author-contact">'.kuas_beta_author_contact_list($user->ID,false,'icon').'</div></div>';
	            $content .= '<p class="more-info-user-groups"><a href="'.get_author_posts_url( $user->ID ).'" class="more-info-icon">info Lengkap &rarr;</a></p>';
	            $content .= '</li>';
	        }
	        $content .= '</ul></div>';
	    }
