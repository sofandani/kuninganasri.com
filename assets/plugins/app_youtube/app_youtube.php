...
//Originial Name: Youtube Channel Gallery - youtube-channel-gallery
...
?>
	...
	private function ytchag_rss_markup( $instance ) {
		...
		if ( empty( $ytchag_user ) ) {
			...
			if ( $ytchag_feed == 'playlist' && $ytchag_feed_order == 'asc' ) {
				...
				$videos_result = $this->get_rss_data ( $ytchag_cache, $transientId, $ytchag_rss_url, $ytchag_cache_time );
				$new_rss = simplexml_load_string( $videos_result['body'] );
		...
		if ( $response_code != 200 ) {
			...
			foreach ( $entries->entry as $entry ) {
				...
				if ( $thumb_count == 0 ) {
					...
					//player
					if ( $ytchag_player == '1' ) {
						//Custom by Ofan Ebob (KuAs)
						//Original just attach require 'templates/player.php';
						if(isset($_GET['yid'])) {
							$youtubeid = $_GET['yid']; $ytchag_autoplay = '&autoplay=1';
						...
						//End custom
						$content.= '<h3 style="margin:10px 0;">'.__('List Video').'</h3>';
					}
				...
				if ( $ytchag_title || $ytchag_description ) {
					...
					if ( $ytchag_title ) {
						//Custom by Ofan Ebob (KuAs)
						//Original just attach:
						//$title_and_description_content.= '<h5 class="ytctitle"><a class="ytclink" href="' . $youtube_url . '/watch?v=' . $youtubeid . '" data-playerid="ytcplayer' . $plugincount . '" data-quality="' . $ytchag_quality . '" alt="' . $title . '" title="' . $title . '" ' . $ytchag_nofollow . '>' . kuas_beta_slice_title_limit($title,5) . '</a></h5>';
						if(isset($_GET['yid'])) $youtubeid = $yt->videoid;
						$title_and_description_content.= '<h5 class="ytctitle"><a class="ytclink" href="' . $youtube_url . '/watch?v=' . $youtubeid . '" data-playerid="ytcplayer' . $plugincount . '" data-quality="' . $ytchag_quality . '" alt="' . $title . '" title="' . $title . '" ' . $ytchag_nofollow . '>' . kuas_beta_slice_title_limit($title,5) . '</a></h5>';
						//End custom
					}
				...
				//Custom by Ofan Ebob (KuAs)
				//Original just attach:
				//$content.= '<a class="ytcthumb ytclink status-'. $ytchag_player .'" ' .$ytchag_thumb_window. ' href="' . $youtube_url . '/watch?v=' . $youtubeid . '" data-playerid="ytcplayer' . $plugincount . '" data-quality="' . $ytchag_quality . '" title="' . $title . '" style="background-image:url(' . $this->check_ssl( $thumb ) . ')" ' . $ytchag_nofollow . '>';
				if($ytchag_player == '1'){
				$youtubeid = $yt->videoid;
				if(empty($_GET['yid'])) $ytchag_autoplay = apply_filters( 'ytchag_autoplay', $instance['ytchag_autoplay'] );
				$content.= '<a class="ytcthumb ytclink status-'. $ytchag_player .'" ' .$ytchag_thumb_window. ' href="' . $youtube_url . '/watch?v=' . $youtubeid . '" data-playerid="ytcplayer' . $plugincount . '" data-quality="' . $ytchag_quality . '" title="' . $title . '" style="background-image:url(' . $this->check_ssl( $thumb ) . ')" ' . $ytchag_nofollow . '>';
				}
				else{
				$content.= '<a class="ytcthumb ytclink normalTip status-'. $ytchag_player .'" ' .$ytchag_thumb_window. ' href="'. site_url('kuas-tv') .'?yid=' . $youtubeid . '" data-yid="' . $youtubeid . '" data-playerid="ytcplayer' . $plugincount . '" data-quality="' . $ytchag_quality . '" title="' . $title . '" style="background-image:url(' . $this->check_ssl( $thumb ) . ')" ' . $ytchag_nofollow . '>';
				}
				//End Custom
