...
function BuildFailConnection(e, t) {
    jQuery(e).html('<div class="error_4square_maps">Oops, kesalahan ' + t + " </div>");
    maps_loading("stop")
}
jQuery(document).ready(function () {
    jQuery(parent_venue).gmap({
        disableDefaultUI: true,
        center: SudutKordinat,
        zoom: ZoomPeta,
        styles: [{
            stylers: [{
                gamma: .75
        ...
        jQuery.getJSON(kuas_ajax_var.kuas_ajax_url, {
            action: "post_venue",
            type: "post",
            cat: "",
            ordby: "rand",
            ...
            if (e.code == 200) {
                ...
                jQuery("form#search_nearby_venue").on("submit", function (e) {
                    ...
                    var n = jQuery(this).attr("action"),
                        r = t.val();
                    var i = jQuery(this).find('input[name="ll"]').val(),
                        s = i.split(",");
                    var o = kuas_ajax_var.kuas_ajax_date,
                        u = kuas_ajax_var.kuas_locale.split("_");
                    ...
                    jQuery.getJSON(m).done(function (e) {
                        ...
                        if (e.response.venues.length > 0) {
                            ...
                            if (n.count() >= 10) {
                                ...
                                jQuery(parent_venue).gmap("find", "markers", {
                                    property: "id",
                                    value: o.id
                                }, function (e, t) {
                                    jQuery(e).triggerEvent("click")
                ...
                jQuery("div#more_venue").clickToggle(function () {
                    jQuery(container_venue).css("float", "none").animate({
                        height: Math.round(window.innerHeight - window.innerHeight * 12 / 100) + "px"
                    }, function () {
                        jQuery("div.sidetop").fadeOut(function () {
                            jQuery(parent_venue).gmap("closeInfoWindow");
                            jQuery(parent_venue).gmap("refresh", "clear");
                            jQuery("form#search_nearby_venue").css({
                                display: "inline"
                            });
                            jQuery("select#spot_kuningan").css({
                                top: "2px",
                                bottom: "inherit",
                                "border-width": "1px"
                            });
                    ...
                    jQuery(this).addClass("zoomOut").removeClass("zoomIn").find("i").text(zoomout_text)
                }, function () {
                    ...
                    jQuery(container_venue).animate({
                        height: "300px"
                    }, function () {
                    ...
                    jQuery(this).addClass("zoomIn").removeClass("zoomOut").find("i").text(zooming_text)
                }).addClass("active")
            } else {
